...
public class BusinessChoiceExcel {
  
	static WebDriver driver;
	XSSFWorkbook wb;
	XSSFSheet sheet1;
	@Test(dataProvider="excelbcdata") 
	public void f(String url) {
		...
		//Lunch the URL
		//String url1 = "https://ecom:ec0m@staging.www.ihg.com/hotels/us/en/global/bc/ericsson?corporateNumber=954299722";
		driver.get(url);
		try {
			Thread.sleep(3000l);
...
@DataProvider(name = "excelbcdata")
public Object[] passData() {
    String excelPath ="C://work//BCURLs.xlsx";
	try{
	...
	ExcelDataConfig config =  new ExcelDataConfig ("C://work//BCURLs.xlsx");
	int rows = config.getRowCounter(0);
	Object[] data = new Object[rows];
	
	for (int i = 0; i<rows; i++) {
		
		data[i]= config.getData(0, i, 1);
		//String data = sheet1.getRow(i).getCell(1).getStringCellValue();
		System.out.println("this is the url for "+"row number "+ i + " " + data);
